...
mcp = FastMCP("prompt-engineering-server")
...
class PromptEngineer:
    ...
    def identify_task_type(self, prompt: str) -> TaskType:
        ...
        keywords = {
            ...
            TaskType.SUMMARIZATION: ["resuma", "resumo", "sintetize", "principais pontos"]
        }
    ...
    def _apply_context(self, prompt: str, context: Dict[str, Any]) -> str:
        ...
        if context.get("target_audience"):
            context_additions.append(f"Público-alvo: {context['target_audience']}")
        
        if context.get("desired_length"):
            context_additions.append(f"Extensão desejada: {context['desired_length']}")
    ...
    def _apply_persona(self, prompt: str, task_type: TaskType) -> str:
        ...
        persona = personas.get(task_type, "Você é um assistente útil e conhecedor")
        return f"{persona}.\n\n{prompt}"
    ...
    def _generate_suggestions(self, task_type: TaskType, prompt: str) -> List[str]:
        ...
        # Sugestões gerais
        if len(prompt) < 20:
            suggestions.append("Considere adicionar mais detalhes sobre o que você deseja")
...
async def analyze_prompt(prompt: str) -> Dict[str, Any]:
    ...
    if any(indicator in prompt.lower() for indicator in context_indicators):
        ...
        analysis["recommendations"].append("Adicione contexto sobre quando, onde ou como")
    
    # Objetivo claro (0-25 pontos)
    action_verbs = ["crie", "analise", "explique", "desenvolva", "resuma", "traduza"]
    if any(verb in prompt.lower() for verb in action_verbs):
        ...
        analysis["recommendations"].append("Use verbos de ação para clarificar o objetivo")
    ...
    if "?" in prompt or "\n" in prompt:
        ...
        analysis["recommendations"].append("Considere usar pontuação ou quebras de linha para estruturar melhor")
    
    analysis["quality_score"] = score
    
    # Recomendações baseadas na pontuação
    if score < 50:
        analysis["recommendations"].insert(0, "Este prompt precisa de melhorias significativas")
    elif score < 75:
        analysis["recommendations"].insert(0, "Prompt razoável, mas pode ser melhorado")
    else:
...
async def check_bias(prompt: str) -> Dict[str, Any]:
    ...
    if gender_bias["masculino"] > 0 and gender_bias["feminino"] == 0:
        biases_found.append("Possível viés de gênero (apenas termos masculinos)")
        mitigations.append("Use linguagem neutra em gênero ou inclua exemplos diversos")
    ...
    for stereotype, neutral in stereotypes.items():
        if stereotype in prompt.lower():
            biases_found.append(f"Termo com potencial estereótipo: '{stereotype}'")
            mitigations.append(f"Considere usar '{neutral}' para maior neutralidade")
    ...
    if any(word in prompt.lower() for word in ["normal", "padrão", "comum"]):
        ...
        mitigations.append("Especifique o contexto cultural ou use termos mais inclusivos")
