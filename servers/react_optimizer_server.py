...
class ReactUnifiedAnalyzer:
    ...
    async def _analyze_design_trends(self, code: str) -> Dict[str, bool]:
        ...
        # Typography trends
        compliance["bold_typography"] = bool(re.search(r'font-weight:\s*(bold|[6-9]00)', code, re.IGNORECASE))
        compliance["variable_fonts"] = "'font-variation-settings'" in code
        compliance["serif_fonts"] = bool(re.search(r'serif|Times|Georgia', code, re.IGNORECASE))
        
        # Design trends
        compliance["dark_mode"] = bool(re.search(r'dark:|dark-|darkMode|dark\s*theme', code, re.IGNORECASE))
        compliance["glassmorphism"] = bool(re.search(r'backdrop-blur|glass|rgba\([^)]*,\s*0\.[0-9]', code))
        compliance["organic_shapes"] = bool(re.search(r'border-radius:\s*[0-9]+%|rounded-full|organic', code))
        compliance["animations"] = bool(re.search(r'@keyframes|animation:|transition:|framer-motion', code))
        compliance["interactive_3d"] = bool(re.search(r'transform.*rotate|perspective|three\.js|@react-three', code))
        
        # Modern UI libraries
        compliance["modern_ui_lib"] = bool(re.search(r'@mui|tailwind|styled-components|framer-motion|shadcn', code))
    ...
    async def _analyze_react_patterns(self, code: str) -> Dict[str, bool]:
        ...
        # Component patterns
        patterns["custom_hooks"] = bool(re.search(r'const\s+use[A-Z][a-zA-Z]*\s*=', code))
        ...
        patterns["functional_components"] = bool(re.search(r'const\s+[A-Z][a-zA-Z]*\s*=.*=>', code))
        patterns["typescript_usage"] = bool(re.search(r':\s*(React\.FC|JSX\.Element|React\.ReactElement)', code))
        ...
        # Compound components
        patterns["compound_components"] = bool(re.search(r'\w+\.\w+', code)) and "children" in code
    ...
    async def _analyze_performance_issues(self, code: str) -> List[str]:
        ...
        if "useEffect(() =>" in code and "[]" not in code:
            issues.append("useEffect sem dependências pode causar re-renders desnecessários")
        
        if re.search(r'new Date\(\)|Date\.now\(\)', code) and "useMemo" not in code:
            issues.append("Operações de data sem memoização podem impactar performance")
        
        if re.search(r'find\(|filter\(|sort\(', code) and "useMemo" not in code:
            issues.append("Operações de array sem memoização podem causar re-computação")
        
        if re.search(r'onClick=\{.*=>', code):
            issues.append("Funções inline podem causar re-renders desnecessários")
    ...
    async def _analyze_accessibility_issues(self, code: str) -> List[str]:
        ...
        if re.search(r'onClick.*<div|onClick.*<span', code):
            issues.append("Elementos não-interativos com onClick - use botões apropriados")
        
        if "tabIndex" not in code and re.search(r'onClick|onKeyDown', code):
            issues.append("Elementos interativos podem precisar de gerenciamento de foco")
        
        if re.search(r'color:\s*#(?:fff|ffffff|000|000000)', code):
            issues.append("Cores de alto contraste detectadas - verifique readabilidade")
        
        if not re.search(r'<h[1-6]', code) and len(code) > 500:
            issues.append("Componente grande sem estrutura de headings adequada")
        
        if "aria-" not in code and len(code) > 200:
            issues.append("Ausência de atributos ARIA para melhor acessibilidade")
    ...
    async def _generate_code_recommendations(
        ...
        if not trends.get("dark_mode", False):
            recommendations.append("💡 Adicione suporte a dark mode seguindo tendências 2025")
        
        if not trends.get("bold_typography", False):
            recommendations.append("🔤 Use typography bold e capitalizada para maior impacto visual")
        
        if not trends.get("animations", False):
            recommendations.append("✨ Adicione micro-animações para melhor UX (considere framer-motion)")
        
        if not trends.get("glassmorphism", False):
            recommendations.append("🌫️ Considere efeitos glassmorphism com backdrop-blur")
        
        # Recomendações baseadas em padrões React
        if not patterns.get("custom_hooks", False):
            recommendations.append("🔧 Extraia lógica repetitiva para custom hooks")
        
        if not patterns.get("memo_usage", False) and len(code) > 300:
            recommendations.append("⚡ Use React.memo para componentes que não mudam frequentemente")
        
        if not patterns.get("typescript_usage", False):
            recommendations.append("🛡️ Migre para TypeScript para melhor type safety")
        
        if not patterns.get("compound_components", False) and any(word in code.lower() for word in ["modal", "form", "accordion"]):
            recommendations.append("🧩 Considere compound components pattern para maior flexibilidade")
        
        # Recomendações de bibliotecas modernas
        if not trends.get("modern_ui_lib", False):
            recommendations.append("📚 Considere usar Tailwind CSS + shadcn/ui para UI components modernos")
        
        # Recomendações baseadas em performance
        if len(performance) > 3:
            recommendations.append("🚀 Foque em otimizações de performance - muitos problemas detectados")
        
        # Recomendações baseadas em acessibilidade
        if len(accessibility) > 2:
            recommendations.append("♿ Priorize melhorias de acessibilidade para inclusão")
    ...
    async def _calculate_code_score(
        ...
        final_score = base_score + trends_bonus + patterns_bonus - performance_penalty - accessibility_penalty
    ...
    async def _identify_applicable_trends(self, prompt: str, component_type: ComponentType) -> List[str]:
        ...
        applicable.extend(["dark_mode_first", "accessibility_first", "glassmorphism"])
...
class ReactUnifiedOptimizer:
    ...
    async def optimize_react_code(
        ...
        if focus_areas is None:
            focus_areas = ["performance", "accessibility", "trends", "patterns"]
        ...
        for area in focus_areas:
            if area == "performance":
                ...
                changes_made.extend([f"Performance: {change}" for change in perf_changes])
    ...
    async def optimize_prompt(
        ...
        if include_accessibility:
            ...
            accessibility_features.extend(["ARIA attributes", "Keyboard navigation", "Screen reader support"])
            enhancements_applied.append("Requisitos de acessibilidade adicionados")
        
        # 6. Performance
        if include_performance:
            perf_section = await self._add_performance_to_prompt(analysis.complexity_level)
            optimized_sections.append(perf_section)
            performance_considerations.extend(["Memoization", "Lazy loading", "Optimized rendering"])
    ...
    async def _optimize_code_performance(self, code: str) -> Tuple[str, List[str]]:
        ...
        if "export default" in code and "React.memo" not in code and len(code) > 200:
            ...
            changes.append("Adicionado React.memo para otimização de re-renders")
        ...
        if re.search(effect_pattern, optimized) and ", [])" not in optimized:
            ...
            changes.append("Adicionado array de dependências vazio ao useEffect")
        ...
        if re.search(r'\.map\(|\.filter\(|\.sort\(', optimized) and "useMemo" not in optimized:
            changes.append("Considere usar useMemo para operações de array computacionalmente pesadas")
    ...
    async def _optimize_code_accessibility(self, code: str) -> Tuple[str, List[str]]:
        ...
        if re.search(button_pattern, optimized):
            changes.append("Considere converter divs clicáveis para elementos button semânticos")
        ...
        if not re.search(r'<h[1-6]', optimized) and len(optimized) > 300:
            changes.append("Adicione estrutura de headings (h1-h6) para melhor navegação")
    ...
    async def _apply_2025_trends_to_code(self, code: str) -> Tuple[str, List[str]]:
        ...
        if "dark:" not in code and "darkMode" not in code:
            changes.append("Considere adicionar suporte a dark mode com classes 'dark:'")
        
        # Melhorar typography
        if "font-bold" not in code and "font-weight" not in code:
            changes.append("Considere aplicar typography bold para maior impacto visual")
        
        # Adicionar micro-animations
        if "transition" not in code and "framer-motion" not in code:
            changes.append("Adicione micro-animações com transitions ou Framer Motion")
    ...
    async def _apply_modern_patterns_to_code(self, code: str) -> Tuple[str, List[str]]:
        ...
        if "useState" in code and "useEffect" in code and "use" not in code[:50]:
            changes.append("Considere extrair lógica para custom hook reutilizável")
        
        # Sugerir TypeScript
        if "React.FC" not in code and "interface" not in code:
            changes.append("Considere migrar para TypeScript para melhor type safety")
        
        # Sugerir compound components
        if any(word in code.lower() for word in ["modal", "form", "accordion"]) and code.count(".") < 3:
            changes.append("Considere compound components pattern para maior flexibilidade")
    ...
    async def _build_context_section(self, component_type: ComponentType, complexity: ComplexityLevel) -> str:
        ...
        base_context = context_templates.get(component_type, "Crie um componente React moderno")
    ...
    async def _enhance_functional_requirements(
        ...
        if analysis.missing_elements:
            missing_additions = {
                "responsive": "Implemente design totalmente responsivo (mobile-first approach)",
                "accessibility": "Garanta acessibilidade completa (WCAG 2.1 AA)",
                "styling": "Use Tailwind CSS para styling moderno e consistente",
                "state_management": "Gerencie estado de forma eficiente com hooks apropriados",
                "typescript": "Use TypeScript com interfaces bem definidas",
                "performance": "Otimize performance com técnicas modernas React"
            }
            
            additions = []
            for missing in analysis.missing_elements:
                if missing in missing_additions:
                    additions.append(f"- {missing_additions[missing]}")
    ...
    async def _add_ui_trends_to_prompt(self, trends: List[str], component_type: ComponentType) -> str:
        ...
        instructions.insert(0, "- Use Tailwind CSS como base: classes utilitárias, responsive design, utility-first")
...
- Component composition following React best practices"""
        ...
        versions["visual_copilot"] = visual_prefix + optimized_prompt + visual_suffix
...
async def analyze_react_code(
    ...
    try:
        ...
        logger.info("React code analyzed", 
        ...
        logger.error("Error analyzing React code", error=str(e))
        raise
...
async def optimize_react_code(
    ...
    try:
        ...
        logger.info("React code optimized", 
                   changes_count=len(optimization.changes_made))
        
        return asdict(optimization)
        
    except Exception as e:
        logger.error("Error optimizing React code", error=str(e))
        raise
...
async def analyze_react_prompt(prompt: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.info("React prompt analyzed", 
                   component_type=analysis.detected_component_type.value,
                   complexity=analysis.complexity_level.value)
        
        return asdict(analysis)
        
    except Exception as e:
        logger.error("Error analyzing React prompt", error=str(e))
        raise
...
async def optimize_react_prompt(
    ...
    try:
        ...
        ai_tool = AITool(target_ai_tool) if target_ai_tool != "generic" else AITool.GENERIC
        ...
        logger.info("React prompt optimized", 
                   enhancements_count=len(optimization.enhancements_applied),
                   target_tool=target_ai_tool)
        
        return asdict(optimization)
        
    except Exception as e:
        logger.error("Error optimizing React prompt", error=str(e))
        raise
...
async def validate_prompt_quality(prompt: str) -> Dict[str, Any]:
    ...
    quality_checks = {
        "clarity": {
            "weight": 20,
            "criteria": [
                ("Objetivo claro", lambda p: len(p.split()) >= 5),
                ("Linguagem específica", lambda p: any(word in p.lower() for word in ["create", "build", "implement", "develop"])),
                ("Sem ambiguidade", lambda p: "?" not in p or p.count("?") <= 2)
            ]
        },
        ...
        "technical_context": {
            "weight": 25,
            "criteria": [
                ("Framework mencionado", lambda p: "react" in p.lower()),
                ("Styling definido", lambda p: any(word in p.lower() for word in ["tailwind", "css", "styled", "mui", "chakra"])),
                ("TypeScript considerado", lambda p: "typescript" in p.lower() or "types" in p.lower()),
                ("State management", lambda p: any(word in p.lower() for word in ["state", "hook", "context", "redux"]))
            ]
        },
        ...
        "ui_ux_considerations": {
            "weight": 20,
            "criteria": [
                ("Responsividade", lambda p: any(word in p.lower() for word in ["responsive", "mobile", "tablet", "desktop"])),
                ("Acessibilidade", lambda p: any(word in p.lower() for word in ["accessible", "a11y", "aria", "keyboard"])),
                ("Estados UI", lambda p: any(word in p.lower() for word in ["loading", "error", "success", "empty"])),
                ("Interações", lambda p: any(word in p.lower() for word in ["hover", "click", "animation", "transition"]))
            ]
        },
        ...
        "modern_practices": {
            "weight": 15,
            "criteria": [
                ("Performance", lambda p: any(word in p.lower() for word in ["performance", "optimization", "lazy", "memo"])),
                ("Padrões 2025", lambda p: any(word in p.lower() for word in ["dark mode", "glassmorphism", "compound"])),
                ("Testing", lambda p: any(word in p.lower() for word in ["test", "testing", "testable"]))
            ]
        },
        ...
        "completeness": {
            "weight": 20,
            "criteria": [
                ("Contexto suficiente", lambda p: len(p.split()) >= 10),
                ("Requisitos específicos", lambda p: any(word in p for word in ["-", "•", "1.", "2.", "3."])),
                ("Exemplo ou caso de uso", lambda p: any(word in p.lower() for word in ["example", "like", "such as", "for instance"]))
            ]
        }
    }
    ...
    for category, config in quality_checks.items():
        category_score = 0
        passed_criteria = []
        failed_criteria = []
        
        for criterion_name, criterion_func in config["criteria"]:
            if criterion_func(prompt):
                category_score += 1
                passed_criteria.append(criterion_name)
            else:
                failed_criteria.append(criterion_name)
        
        # Normalizar score da categoria (0-100)
        max_criteria = len(config["criteria"])
        normalized_score = (category_score / max_criteria) * 100
        weighted_score = (normalized_score * config["weight"]) / 100
        ...
        if failed_criteria:
            improvements.extend([f"{category.title()}: {criterion}" for criterion in failed_criteria])
    ...
    if total_score >= 90:
        ...
        grade = "Inadequado"
        recommendation = "Prompt requer reestruturação completa"
    ...
    return {
        ...
        "analysis_summary": {
            "strengths": [cat for cat, scores in category_scores.items() if scores["score"] >= 75],
            "weaknesses": [cat for cat, scores in category_scores.items() if scores["score"] < 50],
            "most_critical_improvements": improvements[:3] if improvements else []
        }
    }
...
async def generate_component_template(
    ...
    ai_versions = await optimizer._generate_tool_specific_versions(
        base_prompt, AITool(target_ai_tool) if target_ai_tool != "generic" else AITool.GENERIC
    )
...
if __name__ == "__main__":
    ...
    logger.info("Features: Code Analysis + Optimization | Prompt Analysis + Optimization")
    logger.info("Based on: 25+ React templates + UI/UX trends 2025 + AI prompting best practices")
